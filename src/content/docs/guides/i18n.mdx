...
Similar to reusable UI strings, we have labels for the groups displayed in the docs sidebar. For example, the labels for the sidebar tabs are stored in these files.

Navigation labels are stored in the `src/content/nav/` folder, with a TypeScript file for each language. The non-English files should contain a default export using the `navDictionary()` helper method. Just like for UI strings, keys are not translated, only the values in the object passed to `navDictionary()`.
