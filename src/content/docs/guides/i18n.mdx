...
We can currently support the above languages thanks to active community translators willing to volunteer their time to contribute with native speaker translations and reviews!

Unfortunately, we cannot support all languages at this time, and must prioritize based on available community members and their needs. 

All translations are generously contributed by native speaking community volunteers. We do not accept automated/machine translations. (If someone needs a machine translation, they can use the translation feature built in to their browser. There is no additional value to the reader if we provide machine translations, but there is additional maintenance cost for us.)

Adding a language means allocating additional Astro resources to attracting new contributors, and maintaining changes to our code base in multiple languages. **We will not add any new languages that we haven't approved beforehand**, and we cannot approve a new language without a group of people prepared to undertake significant work to translate our entire site.
