...
Import and use the [Starlight `<FileTree />` component](https://starlight.astro.build/components/file-tree/) to produce collapsible file trees in docs. Use hyphens (`-`) at different tab depths to show the file structure.
...
Highlighting is available by writing an item in bold. Inline comments may also be added.
